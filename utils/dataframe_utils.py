...
def binarize(input_path, target_path):
    ...
    df['gender'] = df['gender'].apply(lambda x: 1 if x > 0.9 else 0)
    # df['hold racket handed'] = df['hold racket handed'].apply(lambda x: 1 if x >= 0.9 else 0)
...
binarize(input_path, target_path)